...
// WOYOYO-012 fix: Repliz appends its own "?code=..." to our callback link
// instead of "&code=...". That leaves a second "?" in the address, so the
// normal req.query reader glues everything after our "s" value (including
// "?code=...") onto "s" itself, and the 64-character check below fails.
// This reads the real address by hand and treats every "?" after the first
// one the same as "&", so both our "s" and Repliz's "code" are found.
function readCallbackParams(req) {
  const raw = String(req.url || '');
  const firstQuestion = raw.indexOf('?');
  if (firstQuestion === -1) return {};
  const query = raw.slice(firstQuestion + 1).replace(/\?/g, '&');
  return Object.fromEntries(new URLSearchParams(query));
}

export async function callbackOAuth(req, res, db) {
  const params = readCallbackParams(req);
  const state = String(params.s || req.query?.s || req.query?.stoyangu_state || '');
  ...
  const code = String(params.code || req.query?.code || req.query?.auth_code || req.query?.authorization_code || '');
  const failure = params.error_description || params.error || req.query?.error_description || req.query?.error;
