// The two prompts for the AI builder. Prompt 1 designs freely inside our few
// hard rules; Prompt 2 packages the result as the single-file skin we need.

export const AI_SKIN_DESIGN_PROMPT = `Design a complete Kenyan social-shop storefront called "{STORE}" for a seller of {PRODUCTS}, using my uploaded logo/photos where I provided them.

You have full creative freedom (any aesthetic, fonts, colours, CSS-driven motion and layout), with ONLY these hard rules:
1. Exactly THREE sections, in this order: Home, Products, Contact. Nothing else. No cart, no checkout, no account pages, no backend talk. (There is no grouping/filter section anywhere — the app adds its own universal product search bar inside Products.)
2. Every "order/buy" intent is a WhatsApp link or button — never a cart.
3. All images come from permanent https hosts (Unsplash, Pexels, the ones I attach) — never temp hosts like tmpfiles.org; those expire in hours.
4. Mobile-first, fast, accessible. Treat the store name as a live slot because our app fills it.
5. HTML + CSS ONLY — absolutely no JavaScript. Animations via pure CSS (@keyframes, transitions). The host app strips any <script> tags and inline handlers you emit. Everything interactive is supplied by the host app's own bridge.`;
...
- An EMPTY <div id="stoyangu-products" data-product-grid></div> where the real product catalogue should appear (our app renders the seller's live products there).
