...
//     {{STORE_LOGO}} {{PRODUCTS}} and optionally
...
//     {{PRICE_KES}} {{PRODUCT_IMAGE}} {{GALLERY}} {{COLORS}}
...
export function autoRepairSkin(texts) {
  ...
  if (!template) {
    ...
    repaired.push({
      ...
      content: `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{{PRODUCT_NAME}} — {{STORE_NAME}}</title></head><body><main class="sty-product-page"><a class="sty-back" href="{{BACK_URL}}">← All products</a><article class="sty-product"><div class="sty-gallery">{{GALLERY}}</div><div class="sty-info"><h1>{{PRODUCT_NAME}}</h1><strong class="sty-price">{{PRICE}}</strong><div class="sty-choices">{{COLORS}}{{SIZES}}</div><a class="sty-order" href="{{ORDER_URL}}" data-sty-order data-product="{{PRODUCT_ID}}" target="_blank" rel="noopener noreferrer">Order via WhatsApp</a></div></article><section class="sty-similar-zone">{{SIMILAR}}</section></main></body></html>`,
    });
...
const DEFAULT_CARD = `<article class="sty-card" data-sty-card data-product="{{PRODUCT_ID}}"><a href="{{PRODUCT_URL}}" class="sty-img"><img src="{{PRODUCT_IMAGE}}" alt="{{PRODUCT_NAME}}" loading="lazy"></a><div class="sty-body"><strong class="sty-name">{{PRODUCT_NAME}}</strong><span class="sty-price">{{PRICE}}</span><a class="sty-order" href="{{ORDER_URL}}" target="_blank" rel="noopener noreferrer" data-sty-order data-product="{{PRODUCT_ID}}">Order via WhatsApp</a></div></article>`;
...
export function buildSearchBar() {
  return `<div id="stoyangu-search" data-sty-search="1" role="search"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="7"></circle><path d="m20 20-3.8-3.8"></path></svg><input type="search" placeholder="Search products\u2026" aria-label="Search products" autocomplete="off"></div>`;
}
