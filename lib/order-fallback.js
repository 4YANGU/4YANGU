...
export async function deleteFallbackOrder(supabase, negativeId) {
  const id = Math.abs(Number(negativeId));
  const { data: row } = await supabase.from('store_events').select('id').eq('id', id).eq('event_type', 'confirmed_order').single();
  if (!row) return false;
