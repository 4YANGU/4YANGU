...
//           the real Repliz API.
//    unconfigured — no usable credentials. Connections and publishing fail
//           explicitly; no artificial successful accounts or deliveries.
//
...
export function replizMode() {
  ...
  if (!access || !secret) return 'unconfigured';
  if (/^(dummy|test|mock|changeme|xxx)/i.test(access)) return 'unconfigured';
  return 'live';
}
...
async function replizRequest(path, { method = 'GET', query, body, timeoutMs = 25000 } = {}) {
  const { access, secret } = replizKeys();
  if (!access || !secret) throw new Error('Repliz access and secret keys are not configured. Add REPLIZ_ACCESS_KEY and REPLIZ_SECRET_KEY in Vercel → Settings → Environment Variables, then redeploy.');
...
// WOYOYO-012 fix: this address used to end in "?action=social-callback",
// so Repliz's own "?code=..." became a SECOND "?" instead of the "&" it
// should have used, which broke the address. This address now has no "?"
// at all — the one-time approval code is added as a path piece instead —
// so whatever Repliz appends always forms one clean, valid "?".
export function replizCallbackUrl(req) {
  ...
  return `${scheme}://${host}/api/social-callback`;
}
...
export async function replizListOAuthChoices({ platform, token }) {
  ...
  const docs = Array.isArray(data) ? data : data?.docs || data?.data?.docs || data?.data || data?.pages || data?.channels || data?.organizations || [];
...
export async function replizConnectOAuth({ platform, code, token, selectionId, accountId }) {
  ...
  const inner = (bag.data && typeof bag.data === 'object') ? bag.data : {};
  const acct = (bag.account && typeof bag.account === 'object') ? bag.account : {};
  const newId = String(bag.accountId || bag.account_id || inner.accountId || inner.account_id || bag.id || bag._id || inner.id || inner._id || acct.id || acct._id || acct.accountId || '');
  let account = null;
  if (newId) {
