...
function ProductPageDetails({ product, config, design, onClose, onOrder }: { product: Product; config: Obj; design: Obj; onClose: () => void; onOrder: RendererProps['onOrder'] }) {
  ...
  const [fulfilment, setFulfilment] = useState('Walk in Store');
  const [deliveryAddress, setDeliveryAddress] = useState('');
  const [orderNote, setOrderNote] = useState('');
  const [customerPhone, setCustomerPhone] = useState(() => localStorage.getItem('stoyangu-customer-phone') || '0');
  ...
  return <section className="sj-product-page-detail" aria-label={product.name}>
    <style dangerouslySetInnerHTML={{ __html: [
      ...
      '.sj-product-page-grid .sj-popup-close{position:absolute;top:18px;right:18px;z-index:6;width:44px;height:44px;border:0;border-radius:50%;background:rgba(255,255,255,.96);color:#17261f;display:grid;place-items:center;cursor:pointer;box-shadow:0 10px 26px rgba(8,18,28,.3)}',
      ...
      '.sj-product-page-grid .sj-photo-nav.next{right:14px}',
    ...
    <motion.div className="sj-product-page-grid" style={{ background: String(safeCss(dialog.background) || '#FFFFFF'), borderRadius: safeCss(dialog.radius), boxShadow: String(safeCss(dialog.box_shadow) || '') }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: Number(design.animations?.product_modal?.duration_seconds || .5), ease: design.animations?.product_modal?.easing || [0.16, 1, 0.3, 1] }}>
      ...
      <div className="sj-modal-content" style={mergedStyle(design, config.content_panel)}><span className="sj-modal-category">{product.category}{config.category_suffix ? ` / ${config.category_suffix}` : ''}</span><h2>{product.name}</h2><strong className="sj-modal-price">{formatMoney(product.price)}</strong>
        ...
        <fieldset className="sj-fulfilment"><span className="sj-fulfilment-title">Delivery</span><div><button type="button" className={fulfilment === 'Walk in Store' ? 'selected' : ''} onClick={() => setFulfilment('Walk in Store')}>Walk in Store</button><button type="button" className={fulfilment === 'Delivery' ? 'selected' : ''} onClick={() => setFulfilment('Delivery')}>Delivery</button></div>{fulfilment === 'Delivery' && <label>Delivery address<textarea value={deliveryAddress} onChange={(event) => setDeliveryAddress(event.target.value)} maxLength={300} placeholder="Estate, building and nearest landmark" /></label>}<label>Optional order note<textarea value={orderNote} onChange={(event) => setOrderNote(event.target.value)} maxLength={300} placeholder="Any extra request or question?" /></label></fieldset>
        ...
        <button className="sj-modal-order" style={{ background: '#19A45B' }} onClick={() => { const saved = localStorage.getItem('stoyangu-customer-phone') || ''; const digits = saved.replace(/\D/g, ''); if (digits.length >= 9 && digits.length <= 15) { onOrder(product, colour, size, fulfilment, finalNote, saved); return; } setCustomerPhone(customerPhone || '0'); setPhoneStepOpen(true); }}><MessageCircle /><span>Order via WhatsApp</span><ArrowRight /></button>
        {config.order_note && <p className="sj-order-note">{String(config.order_note)}</p>}
        <AnimatePresence>{phoneStepOpen && <motion.div className="sj-phone-step-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setPhoneStepOpen(false)}><motion.div className="sj-phone-step" initial={{ opacity: 0, y: 18, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12 }} onClick={(event) => event.stopPropagation()}><button className="sj-phone-step-close" onClick={() => setPhoneStepOpen(false)} aria-label="Close phone form"><X /></button><h3>Where can we reach you?</h3><p>Press confirm, WhatsApp will open with your order ready — Then hit send and we will reply to you shortly.</p><label className="sj-customer-phone"><span>Phone number</span><input type="tel" inputMode="numeric" autoComplete="tel" name="tel" value={customerPhone} onChange={(event) => { setCustomerPhone(event.target.value.replace(/[^0-9+\s-]/g, '')); setPhoneError(''); }} placeholder="0712 345 678 or 0112 345 678" aria-invalid={Boolean(phoneError)} autoFocus /><small>{phoneError}</small></label><button className="sj-phone-confirm" onClick={() => { const digits = customerPhone.replace(/\D/g, ''); if (digits.length < 10 || digits.length > 12 || (!digits.startsWith('07') && !digits.startsWith('01') && !digits.startsWith('254'))) { setPhoneError('Enter a valid phone number.'); return; } localStorage.setItem('stoyangu-customer-phone', customerPhone.trim()); onOrder(product, colour, size, fulfilment, finalNote, customerPhone.trim()); }}>Confirm</button></motion.div></motion.div>}</AnimatePresence>
      </div>
    </motion.div>
  </section>;
}
