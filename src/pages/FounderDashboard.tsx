...
function StorefrontHtmlField({ storeId, value, onChange, onError, onInfo, isFounder }: {
  storeId?: number;
  value: string;
  ...
  if (!isFounder) {
    return (
      <div className="form-intro" style={{ background: '#f8f5ef', padding: 16, borderRadius: 10, border: '1px solid #e6dcc8' }}>
        <strong>Storefront HTML is a founder-only field.</strong>
        <p style={{ margin: '6px 0 0' }}>Ask the StoYangu founder to paste the AI-built HTML template here. The store owner can manage products, photos and order tracking — but the storefront design is set centrally by the platform founder.</p>
      </div>
    );
  }
  return (
...
export default function FounderDashboard() {
  ...
  const becomeFounder = async () => {
    setBootstrapBusy(true); setBootstrapMsg('');
    try {
      const res = await apiFetch<{ ok: boolean; error?: string }>('/api/dashboard?action=bootstrap-founder', { method: 'POST' });
      if (res.ok) { setBootstrapMsg('You are now the founder. Reloading…'); setTimeout(() => window.location.reload(), 1200); }
      else setBootstrapMsg(res.error || 'Could not become founder.');
    } catch (err) {
      setBootstrapMsg(err instanceof Error ? err.message : 'Could not become founder.');
  ...
  return <div className="dashboard-shell"><aside className="dashboard-sidebar"><BrandLogo /><div className="founder-chip"><span>F</span><div><strong>Founder</strong><small>Shared workspace</small></div></div><nav><a className="active" href="#overview"><LayoutDashboard /> Overview</a><a href="#applications"><FilePlus2 /> Applications</a><a href="#stores"><StoreIcon /> All stores</a><button onClick={() => setNotifications(true)}><BellRing /> Notifications</button><button onClick={copyGeneralPrompt}><Sparkles /> {aiPromptCopied ? 'AI prompt copied!' : 'Copy AI prompt'}</button></nav><button className="sidebar-signout" onClick={handleSignOut}><Settings2 /> Sign out</button></aside><main className="dashboard-main"><div className="mobile-topbar"><BrandLogo /><div className="mobile-topbar-actions"><button onClick={copyGeneralPrompt}><Sparkles /> AI prompt</button><button onClick={() => setNotifications(true)}><BellRing /> Alerts</button><button onClick={handleSignOut}><LogOut /> Sign out</button></div></div><header className="dashboard-top"><div><span className="eyebrow">Founder dashboard</span><h1>StoYangu overview</h1><p>Everything moving across the platform today.</p></div><div className="top-actions"><button className="secondary-button" onClick={copyGeneralPrompt}><Sparkles /> {aiPromptCopied ? 'AI prompt copied!' : 'Copy AI prompt'}</button><button className="secondary-button" onClick={() => setNotifications(true)}><BellRing /> Notifications</button><button className="button-primary compact" onClick={() => setNewStore(true)}><Plus /> New store</button></div></header>
    ...
    {!isFounder && !bootstrapMsg && (
      <div className="founder-bootstrap-banner">
        <div>
          <strong>You're signed in as a store owner.</strong>
          <p>To paste the AI-built storefront HTML, your account needs founder access. If you're the platform founder, click below to claim it (one-time, locked to the first claim).</p>
        </div>
        <button className="button-primary" onClick={becomeFounder} disabled={bootstrapBusy}>{bootstrapBusy ? 'Setting up…' : 'Make me the founder'}</button>
      </div>
    )}
    {bootstrapMsg && <div className="founder-bootstrap-banner success"><strong>{bootstrapMsg}</strong></div>}
...
// ---------------------------------------------------------------------------
function NewStoreModal({ isFounder, onClose, onSaved }: { isFounder: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title="Create a new store" onClose={onClose} wide><form className="admin-form" onSubmit={submit}>
    ...
    <StorefrontHtmlField isFounder={isFounder} value={template} onChange={setTemplate} onError={setError} onInfo={setInfo} />
...
// ---------------------------------------------------------------------------
function EditStoreModal({ isFounder, store, onClose, onSaved }: { isFounder: boolean; store: Store; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={`Edit store details: ${store.name}`} onClose={onClose} wide><form className="admin-form" onSubmit={submit}>
    ...
    <StorefrontHtmlField isFounder={isFounder} storeId={store.id} value={template} onChange={setTemplate} onError={setError} onInfo={setInfo} />
    {error && <div className="form-error">{error}</div>}
    {info && <div className="form-success">{info}</div>}
