...
export default function FounderDashboard() {
  ...
  const toggle = async (store: Store) => {
    try {
      if (store.is_active) {
        const confirmed = window.confirm(`Turn OFF ${store.name}?\n\nAll of its orders will be saved safely to the archive and removed from the live store. A download of those orders will start now, and you can restore them when you turn the store back on.`);
        if (!confirmed) return;
        const result = await apiFetch('/api/stores', { method: 'PUT', body: JSON.stringify({ action: 'billing', id: store.id, is_active: false }) }) as { archived_orders?: Array<Record<string, unknown>> };
        downloadArchivedOrders(store, result.archived_orders || []);
        await load();
        return;
      }
  ...
  return <div className="dashboard-shell"><aside className="dashboard-sidebar"><BrandLogo /><div className="founder-chip"><span>F</span><div><strong>Founder</strong><small>Shared workspace</small></div></div><nav><a className="active" href="#overview"><LayoutDashboard /> Overview</a><a href="#applications"><FilePlus2 /> Applications</a><a href="#stores"><StoreIcon /> All stores</a><button onClick={() => setNotifications(true)}><BellRing /> Notifications</button><button onClick={copyClientPrompt}><Clipboard /> Client AI prompt</button></nav><button className="sidebar-signout" onClick={handleSignOut}><Settings2 /> Sign out</button></aside><main className="dashboard-main"><div className="mobile-topbar"><BrandLogo /><div className="mobile-topbar-actions"><button onClick={() => setNotifications(true)}><BellRing /> Alerts</button><button onClick={handleSignOut}><LogOut /> Sign out</button></div></div><header className="dashboard-top"><div><span className="eyebrow">Founder dashboard</span><h1>StoYangu overview</h1><p>Everything moving across the platform today.</p></div><div className="top-actions"><button className="secondary-button source-download update-pack-button" onClick={copyClientPrompt}>{clientPromptCopied ? <><Check /> Prompt copied!</> : <><Clipboard /> Copy new-client AI prompt</>}</button><button className="secondary-button" onClick={() => setNotifications(true)}><BellRing /> Notifications</button><button className="button-primary compact" onClick={() => setNewStore(true)}><Plus /> New store</button></div></header>
    <a className="update-pack-banner" href="/stoyangu-vfinal-pack.zip?download=vfinal" download="stoyangu-vfinal-pack.zip">
      <Download />
      <div>
        <strong>Update pack (FREE/PRO) — VFINAL PACK: the complete final update</strong>
        <span>FREE plan is now 7 orders per 30 days (counter, website and all AI-scraper info updated); store power button now archives orders safely with download + restore-or-blank on reactivation and a view-archived button; latest update and Download Stika button moved before/next to the right places in Manage Store; popup delivery section renamed "Ikufikie aje?"; WhatsApp text no longer repeats "Delivery"; new client AI prompt with desktop navbar (logo left, menu middle, name right). Only the listed files — nothing else.</span>
      </div>
      <em>Download update</em>
    </a>
    <a className="update-pack-banner" href="/stoyangu-tracker-removal-update.zip" download>
      <Download />
      <div>
        <strong>Previous pack — full tracker cleanup (element-picker, rrweb, beacon)</strong>
        <span>Removes ALL leftover platform tooling from index.html and guards every future build. Apply this first if you have not already.</span>
      </div>
      <em>Download update</em>
    </a>
