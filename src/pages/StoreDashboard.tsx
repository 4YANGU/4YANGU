import { ArrowLeft, BellRing, Check, Edit3, ExternalLink, Eye, EyeOff, Home, Inbox as InboxIcon, KeyRound, LogOut, MessageCircle, Plus, RefreshCw, Smartphone, Store as StoreIcon, Trash2, Users, X } from 'lucide-react';
...
export default function StoreDashboard() {
  ...
  return <div className="owner-page"><header className="owner-header owner-header-split"><div className="owner-header-actions"><span>{profile?.role === 'founder' ? 'Founder manage view' : 'StoYangu'}</span><div>{profile?.role === 'founder' && <button onClick={() => window.location.assign('/founder')} style={{ background: '#16a34a', color: '#fff', border: 0, borderRadius: 999, padding: '.5rem .9rem', fontWeight: 800, cursor: 'pointer' }}><ArrowLeft /> Back to founder dashboard</button>}{profile?.role === 'owner' && !appInstalled && <button onClick={() => setInstallOpen(true)}><Smartphone /> Install app</button>}{profile?.role === 'owner' && <button onClick={() => setPasswordOpen(true)}><KeyRound /> Change password</button>}<button onClick={signOut}><LogOut /> Sign out</button></div></div><div className="owner-header-identity">{store.logo_url ? <img className="owner-store-logo" src={store.logo_url} alt={`${store.name} logo`} /> : <span className="owner-store-logo-fallback"><BrandLogo compact /></span>}<div className="owner-header-copy"><h1>{store.name}</h1><span className="owner-cycle-dates">{cycleLabel}</span><div className="owner-store-link-row"><a href={storeLink(store.slug)} target="_blank" rel="noreferrer" onClick={handleStorefrontClick}>{storeDomain(store.slug)} <ExternalLink /></a><button onClick={copyStoreLink}>{linkCopied ? 'Copied!' : 'Copy link'}</button></div></div></div></header><main className="owner-main">
    {/* Woyoyo-003: Home + Products merged into one page — tabs retired. */}
