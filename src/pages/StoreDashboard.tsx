import { ArrowLeft, BellRing, Camera, Check, Edit3, ExternalLink, Eye, EyeOff, Image, KeyRound, LogOut, MessageCircle, PackagePlus, Plus, RefreshCw, Smartphone, Store as StoreIcon, Trash2, Users, X } from 'lucide-react';
...
export default function StoreDashboard() {
  ...
  </main>{passwordOpen && <PasswordChangeModal onClose={() => setPasswordOpen(false)} />}{editing && <ProductModal product={editing === 'new' ? null : editing} storeId={store.id} onClose={() => setEditing(null)} onSaved={async () => { setEditing(null); await load(); }} />}</div>;
}
...
function ProductModal({ product, storeId, onClose, onSaved }: { product: Product | null; storeId: number; onClose: () => void; onSaved: () => void }) {
