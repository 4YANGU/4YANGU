...
export default function StoreDashboard() {
  ...
  const updateOrderStatus = async (order: Order, status: Order['status']) => { try { await apiFetch('/api/orders', { method: 'PUT', body: JSON.stringify({ id: order.id, status }) }); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not update that order.'); } };
  ...
  return <div className="owner-page"><header className="owner-header"><div className="owner-header-actions"><span>{profile?.role === 'founder' ? 'Founder manage view' : 'My StoYangu'}</span><div>{profile?.role === 'owner' && !isStandaloneApp() && <button onClick={() => setInstallOpen(true)}><Smartphone /> Install app</button>}{profile?.role === 'owner' && <button onClick={() => setPasswordOpen(true)}><KeyRound /> Change password</button>}<button onClick={signOut}><LogOut /> Sign out</button></div></div><BrandLogo compact /><h1>{store.name}</h1><span className="owner-cycle-dates">{cycleLabel}</span><a href={storeLink(store.slug)} target="_blank" rel="noreferrer" onClick={handleStorefrontClick}>{storeDomain(store.slug)} <ExternalLink /></a></header><main className="owner-main">
    ...
    <section className="incoming-orders"><div className="incoming-orders-head"><div><span className="eyebrow">Order management</span><h2>Incoming orders</h2><p>Customer details and product choices, newest first.</p></div><span className="order-status-count">{(data.orders || []).filter((order) => order.status === 'new').length} new</span></div>{data.orders?.length ? <div className="incoming-order-list">{data.orders.map((order) => <article className="incoming-order" key={order.id}><div className="incoming-order-main"><strong>{order.product_name}</strong><span>{formatMoney(order.product_price)}{order.color ? ` · ${order.color}` : ''}{order.size ? ` · ${order.size}` : ''}{order.fulfilment ? ` · ${order.fulfilment}` : ''}</span>{order.note && <span>“{order.note}”</span>}</div><div className="incoming-order-customer"><a href={`tel:${order.customer_phone}`}>{order.customer_phone}</a><small>{new Date(order.created_at).toLocaleString('en-KE', { day: 'numeric', month: 'short', hour: 'numeric', minute: '2-digit' })}</small></div><select className="order-status-select" value={order.status} onChange={(event) => updateOrderStatus(order, event.target.value as Order['status'])} aria-label={`Status for order ${order.id}`}><option value="new">New</option><option value="contacted">Contacted</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></article>)}</div> : <div className="orders-empty">Confirmed customer orders will appear here.</div>}</section>
