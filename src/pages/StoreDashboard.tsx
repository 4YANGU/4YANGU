import { ArrowLeft, BellRing, Check, Copy, Edit3, ExternalLink, Eye, EyeOff, Inbox as InboxIcon, KeyRound, LogOut, MessageCircle, Plus, RefreshCw, ShoppingBag, Smartphone, Store as StoreIcon, Trash2, Users, X } from 'lucide-react';
...
export default function StoreDashboard() {
  ...
  const copyStoreLink = async () => { const link = storeLink(store.slug); try { await navigator.clipboard.writeText(link); setLinkCopied(true); window.setTimeout(() => setLinkCopied(false), 1800); } catch { window.prompt('Copy your store link:', link); } };
  ...
  return <div className="owner-page"><header className="owner-header owner-header-split"><div className="owner-header-actions"><span>{profile?.role === 'founder' ? 'Founder manage view' : 'StoYangu'}</span><div>{profile?.role === 'founder' && <button onClick={() => window.location.assign('/founder')} style={{ background: '#16a34a', color: '#fff', border: 0, borderRadius: 999, padding: '.5rem .9rem', fontWeight: 800, cursor: 'pointer' }}><ArrowLeft /> Back to founder dashboard</button>}{profile?.role === 'owner' && !appInstalled && <button onClick={() => setInstallOpen(true)}><Smartphone /> Install app</button>}{profile?.role === 'owner' && <button onClick={() => setPasswordOpen(true)}><KeyRound /> Change password</button>}<button onClick={signOut}><LogOut /> Sign out</button></div></div><div className="owner-header-identity">{store.logo_url ? <img className="owner-store-logo" src={store.logo_url} alt={`${store.name} logo`} /> : <span className="owner-store-logo-fallback"><BrandLogo compact /></span>}<div className="owner-header-copy"><div className="owner-name-row"><h1>{store.name}</h1><button className="owner-copy-link" onClick={copyStoreLink} aria-label="Copy store link" title={linkCopied ? 'Copied!' : 'Copy store link'}>{linkCopied ? <Check /> : <Copy />}</button></div><span className="owner-cycle-dates">{cycleLabel}</span><div className="owner-header-stats"><span className="owner-mini-stat"><Users /> {store.visitor_total.toLocaleString()} visits <small>+{store.visitor_today} today</small></span><span className="owner-mini-stat"><MessageCircle /> {upkeepOrders} orders · 30 days <small>{upkeep.upkeep_plan === 'PAID' ? 'KES 300' : 'FREE TRIAL'}</small></span><a className="owner-visit-store" href={storeLink(store.slug)} target="_blank" rel="noreferrer" onClick={handleStorefrontClick} aria-label="Open storefront" title={storeDomain(store.slug)}><ExternalLink /></a></div></div></div></header><main className="owner-main">
